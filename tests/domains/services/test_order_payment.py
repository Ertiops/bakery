...
async def test__create(
    ...
    order_payment = await order_payment_service.create(input_dto=create_data)
...
async def test__get_last(
    ...
    order_payment = await order_payment_service.get_last()
    assert order_payment == OrderPayment(
...
async def test__get_last__entity_not_found_exception(
    ...
    with pytest.raises(EntityNotFoundException):
        await order_payment_service.get_last()
...
async def test__update_by_id(
    ...
    order_payment = await order_payment_service.update_by_id(input_dto=update_data)
    assert order_payment == OrderPayment(
        id=db_order_payment.id,
...
async def test__update_by_id__entity_not_found_exception(
    ...
    with pytest.raises(EntityNotFoundException):
        await order_payment_service.update_by_id(
