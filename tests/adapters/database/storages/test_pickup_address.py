...
async def test__create(
    pickup_address_storage: PickupAddressStorage,
) -> None:
    create_data = CreatePickupAddress(
        name="test_name",
    )
    pickup_address = await pickup_address_storage.create(input_dto=create_data)
...
async def test__create__entity_already_exists_exception(
    ...
    with pytest.raises(EntityAlreadyExistsException):
        await pickup_address_storage.create(input_dto=create_data)


async def test__get_by_id(
    pickup_address_storage: ProductStorage,
    create_pickup_address: Callable,
) -> None:
    db_pickup_address: PickupAddressTable = await create_pickup_address()
    pickup_address = await pickup_address_storage.get_by_id(
...
async def test__get_by_id__none(pickup_address_storage: PickupAddressStorage) -> None:
    assert await pickup_address_storage.get_by_id(input_id=uuid4()) is None
...
async def test__get_by_id__deleted(
    ...
    assert await pickup_address_storage.get_by_id(input_id=db_pickup_address.id) is None
...
async def test__get_list(
    ...
    pickup_addresses = await pickup_address_storage.get_list(
        input_dto=PickupAddressListParams(limit=10, offset=0)
    )
    assert pickup_addresses == [
        PickupAddress(
            id=db_address.id,
        ...
        for db_address in db_pickup_addresses
    ]
...
async def test__count(
    ...
    count = await pickup_address_storage.count(
        input_dto=PickupAddressListParams(limit=10, offset=0)
    )
    assert count == 1
...
async def test__exists_by_id(
    ...
    db_pickup_address: PickupAddressTable = await create_pickup_address()
    assert await pickup_address_storage.exists_by_id(input_id=db_pickup_address.id)


async def test__exists_by_id__false(
    pickup_address_storage: PickupAddressStorage,
) -> None:
    assert await pickup_address_storage.exists_by_id(input_id=uuid4()) is False
...
async def test__exists_by_id__deleted(
    ...
    assert (
        await pickup_address_storage.exists_by_id(input_id=db_pickup_address.id)
        is False
    )
...
async def test__update_by_id(
    ...
    db_address: PickupAddressTable = await create_pickup_address()
    update_data = UpdatePickupAddress(
        id=db_address.id,
        name="test_name",
    )
    pickup_address = await pickup_address_storage.update_by_id(input_dto=update_data)
    assert pickup_address == PickupAddress(
        id=db_address.id,
...
async def test__update_by_id__entity_already_exists_exception(
    ...
    update_data = UpdatePickupAddress(
        id=db_addresses[0].id,
        name=db_addresses[1].name,
    )
    with pytest.raises(EntityAlreadyExistsException):
        await pickup_address_storage.update_by_id(input_dto=update_data)
...
async def test__update_by_id__entity_not_found_exception(
    ...
    with pytest.raises(EntityNotFoundException):
        await pickup_address_storage.update_by_id(
...
async def test__delete_by_id(
    ...
    db_address: PickupAddressTable = await create_pickup_address()
    await pickup_address_storage.delete_by_id(input_id=db_address.id)
    assert db_address.deleted_at is not None


async def test__delete_by_id__none(
    pickup_address_storage: PickupAddressStorage,
) -> None:
    assert await pickup_address_storage.delete_by_id(input_id=uuid4()) is None
